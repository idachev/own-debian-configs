...
# Postgres Schema Download Script
...
# Optional output file (defaults to schema_name_timestamp.dump)
OUTPUT_FILE="${OUTPUT_FILE:-${SCHEMA_NAME}_$(date +%Y%m%d_%H%M%S).dump}"

echo "Downloading data from schema '$SCHEMA_NAME' in database '$DB_NAME' at $DB_HOST:$DB_PORT..."
echo "Using Postgres ${PG_VERSION} Docker image..."

# Use Docker with Postgres image for pg_dump with custom binary format
docker run --rm \
    $DOCKER_EXTRA_ARGS \
